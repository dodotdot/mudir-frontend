...
const schema = Yup.object().shape({
    name: Yup.string()
        .required('Judul Pelajaran harus diisi'),
    book: Yup.string()
...
async function onSubmit(values) {
    try {
        ...
        if (subject) {
            ...
            message = 'Pelajaran updated';
        } else {
            await subjectsStore.setSubject(values);
            message = 'Pelajaran added';
        }
        await router.push('/list-lesson');
...
<template>
  <div class="p-4 text-white h-1/2 max-w-7xl mx-auto">
    <template v-if="!(subject?.loading || subject?.error)">
      <Form @submit="onSubmit" :validation-schema="schema" :initial-values=" status === 'new' ? subject : subject.data " v-slot="{ errors, isSubmitting }">
        <div class="w-full text-sm mb-5">
          ...
          <div class="relative mb-6">
              <Field name="duration" type="number" id="duration" class="block px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-200 bg-transparent rounded-full border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-200 dark:focus:border-white focus:outline-none focus:ring-0 focus:border-gray-300 peer" placeholder=" " />
          ...
          <Field name="type" type="hidden" value="PELAJARAN"></Field>
        </div>
