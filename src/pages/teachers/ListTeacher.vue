...
watch(searchTerm, (current, old) => {
    if(current !== '') {
        teachersStore.getAllUserByNameTeacher(current)
    } else {
        teachersStore.getAll();
    }
})
  ...
  <div class="p-4 text-white h-1/2 max-w-7xl mx-auto">
    ...
    <div>
       <label for="default-search" class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-gray-300">Search</label>
        <div class="relative">
            <input type="search" v-model="searchTerm"  id="default-search" class="block p-4 pl-4 w-full text-sm text-gray-200 bg-transparent rounded-full border border-gray-300 focus:ring-white focus:border-white dark:bg-gray-200 dark:border-gray-200 dark:placeholder-gray-200 dark:text-white dark:focus:ring-white dark:focus:border-white placeholder:text-white" placeholder="Nama Ustadz">
